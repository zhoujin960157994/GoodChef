...
-(void) GetTokenWithTokenBlock:(TokenBlock)tokenBlock
{
    NSString        *url;
    url         = [NSString stringWithFormat:@"%@?grant_type=client_credential&appid=%@&secret=%@", tokenUrl, appid, appsecret];
    //发送http请求
    [TenpayUtil httpSendJson:url method:@"GET" data:@"" withSucceessBlock:^(id resParams) {
...
//提交预支付
-(void)sendPrepay:(NSMutableDictionary *)prePayParams withPayIdBlock:(PayIdBlock)payBlock
{
    //获取提交支付json
    if (prePayParams) {
        ...
        //发送请求post json数据
        [TenpayUtil httpSendJson:url method:@"POST" data:json withSucceessBlock:^(id resParams) {
            //判断返回
            long retcode   = [[resParams objectForKey:@"errcode"] longValue];
            
            if ( retcode == 0) {//返回成功
                NSString *prepayid = [resParams objectForKey:@"prepayid"];
                payBlock(prepayid);
            }
            else
            {
